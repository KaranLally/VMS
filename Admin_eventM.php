...
$searchResults = [];
$sql = "SELECT event_id, time, date, event_type FROM events"; // base query
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($searchId)) {
        $conditions[] = "event_id = :searchId";
        $params[':searchId'] = [$searchId, PDO::PARAM_INT];
    }
    if (!empty($searchType) && $searchType !== "null" && $searchType !== "all") {
        $conditions[] = "event_type = :searchType";
        $params[':searchType'] = [$searchType, PDO::PARAM_STR];
    }
